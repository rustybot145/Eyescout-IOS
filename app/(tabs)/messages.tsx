...
export default function MessagesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    if (id && !uid) setUid(id);
    if (!id) return;
    const data = await fetchThreads(id);
    setThreads(data);
  }, [uid]);
  ...
  // Open a thread and mark its coach messages read (persisted, like the web).
  const openThread = useCallback(
    (t: Thread) => {
      let changed = false;
      const msgs = t.messages.map((m) => {
        if (m.from === 'coach' && !m.read) {
          changed = true;
          return { ...m, read: true };
        }
        return m;
  ...
  const onSend = useCallback(
    async (text: string) => {
      if (!active) return;
      const msg: ChatMsg = { from: 'player', text, timestamp: new Date().toISOString(), read: true };
...
function ChatView({
  ...
  // Build renderable rows with day dividers + grouping flags.
  ...
  return (
    ...
    >
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const isMine = item.m.from === 'player';
          return (
            <View>
        ...
        ListEmptyComponent={
          <Text style={styles.chatEmpty}>Start of your conversation with {thread.coachName || 'this coach'}.</Text>
        }
      />
      ...
      <View style={[styles.inputWrap, { paddingBottom: insets.bottom + 10 }]}>
        <TextInput
          ...
          placeholder={`Message ${thread.coachName || 'coach'}…`}
...
function EmptyMessages() {
  return (
    <View style={styles.empty}>
      ...
      <Text style={styles.emptyBody}>When a coach reaches out, their message will appear here. Keep your profile updated so coaches can find you.</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  convoAvatar: { width: 48, height: 48 },
  coachBadge: {
  ...
  sendBtn: {
    width: 42, height: 42, borderRadius: 21, backgroundColor: colors.blue,
